...
#Figuer 2####
library(forestplot)
forestplot(labeltext=as.matrix(m[,c(3)]),mean=m$v4,lower=m$v5,upper=m$v6,is.summary=c(T,T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10)),
           graph.pos=2,graphwidth=unit(70,"mm"),boxsize=0.2,line.margin=unit(0.5,"mm"),lineheight=unit(5,"mm"),
...
forestplot(labeltext=as.matrix(m[,c(8)]),mean=m$v9,lower=m$v10,upper=m$v11,is.summary=c(T,T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10)),
           graph.pos=2,graphwidth=unit(70,"mm"),boxsize=0.2,line.margin=unit(0.5,"mm"),lineheight=unit(5,"mm"),
           col=fpColors(box = "grey0",lines = "grey0",zero="black"),colgap=unit(5,"mm"),txt_gp = fpTxtGp(title=gpar(cex=1.2),ticks=gpar(cex=1)),
           title = "CVD mortality",zero=1,xticks=c(0.5,1.0,1.5,2.0,2.5,3.0,3.5),ci.vertices=T)
forestplot(labeltext=as.matrix(m[,c(13)]),mean=m$v14,lower=m$v15,upper=m$v16,is.summary=c(T,T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10),T,rep(F,10)),
           graph.pos=2,graphwidth=unit(70,"mm"),boxsize=0.2,line.margin=unit(0.5,"mm"),lineheight=unit(5,"mm"),
           col=fpColors(box = "grey0",lines = "grey0",zero="black"),colgap=unit(5,"mm"),txt_gp = fpTxtGp(title=gpar(cex=1.2),ticks=gpar(cex=1)),
...
forestplot(labeltext=as.matrix(l[,c(1:3)]),mean=l$v4,lower=l$v5,upper=l$v6,is.summary=c(T,T,T,rep(F,10),T,T,rep(F,30)),
           graph.pos=4,graphwidth=unit(100,"mm"),boxsize=0.2,line.margin=unit(0.5,"mm"),lineheight=unit(6,"mm"),
           col=fpColors(box = "grey0",lines = "grey0",zero="black"),colgap=unit(5,"mm"),txt_gp = fpTxtGp(title=gpar(cex=1.2),ticks=gpar(cex=1)),
